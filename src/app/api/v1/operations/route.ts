...
export async function GET(req: NextRequest) {
  try {
    ...
    let url = EXTERNAL_ROUTES.OPERATIONS.BASE;
    if (date) {
      url += `?date=${date}`;
    }
    ...
    try {
      ...
      data = { message: responseText || 'Error al obtener operaciones' };
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(EXTERNAL_ROUTES.OPERATIONS.BASE, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': authHeader,
    ...
    try {
      ...
      // Si no es JSON, devolver el texto como mensaje
      data = { message: responseText || 'Daily operation created successfully' };
    }
