...
export async function GET(req: NextRequest) {
  try {
    ...
    let url = EXTERNAL_ROUTES.ACTIVITIES.BASE;
    if (active !== null) {
      url += `?active=${active}`;
    }
    ...
    try {
      ...
      data = { message: responseText || 'Error al obtener actividades' };
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(EXTERNAL_ROUTES.ACTIVITIES.BASE, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': authHeader,
    ...
    try {
      ...
      // Si no es JSON, devolver el texto como mensaje
      data = { message: responseText || 'Activity created successfully' };
    }
